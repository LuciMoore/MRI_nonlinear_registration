...
def main():
    ...
    randint = subjectid
    warped_dir = os.path.join('./jlf_slurm_tempdebug3', 'jlf{}'.format(randint))
    ...
    register(warped_dir, subject_image, atlas_images, atlas_segmentations)
...
def register(warped_dir, subject_T1w, atlas_images, atlas_segmentations):
    sub_Tw1_list = []
    ...
    wf.write_graph()
    output = wf.run(plugin='SLURM', plugin_args={'sbatch_args': '-A fnl_lab --mem-per-cpu 8G --time 36:00:00 --cpus-per-task 10 --output %j_debug_output.txt --error %j_debug_error.txt'})
